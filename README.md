...
> Актуально на 13.09.2026: ниже сохранена историческая инструкция старого
> прямого транспорта. **Не используйте её для продакшена.** Текущая схема —
> сборщик GitHub → сервер PostgreSQL → отдельные worker'ы Telegram.
> Worker'ы сейчас отсутствуют, автоматическая доставка остановлена.
> См. `TECHNICAL_AUDIT_2026-09-13.md`, `PROJECT_CONTEXT.md` и `server/OPERATIONS.md`.
> Локальные исправления ещё не развёрнуты. Снятие CENTRAL_INGEST_URL не является откатом.

Локальная проверка: создайте Python 3.12 venv, установите
`server/requirements.txt`, выполните `python -m unittest discover -v`.
Тесты не отправляют Telegram-сообщения и не подключаются к production DB.
